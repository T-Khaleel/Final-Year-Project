...
ann.fit(a, O_tr['Mobile TP'], epochs=200, validation_split=0.2, verbose=1, batch_size=64)
...
t=time.time()
pred['Mobile TP']=ann.predict(b)
disaggregationTime=time.time()-t
pred1['Mobile TP']=ann.predict(a)

#Printing average of R-Squared, MSE, RMSE, MAE of all models
#Checking bias and variance 
print('Test set')     
print(r2_score(O_ts['Mobile TP'],pred['Mobile TP']))
print(mean_squared_error(O_ts['Mobile TP'].values,pred['Mobile TP']))
print(np.sqrt(mean_squared_error(O_ts['Mobile TP'].values,pred['Mobile TP'])))
print(mean_absolute_error(O_ts['Mobile TP'],pred['Mobile TP']))

print('Train set')
print(r2_score(O_tr['Mobile TP'],pred1['Mobile TP']))
print(mean_squared_error(O_tr['Mobile TP'].values,pred1['Mobile TP']))
print(np.sqrt(mean_squared_error(O_tr['Mobile TP'].values,pred1['Mobile TP'])))
print(mean_absolute_error(O_tr['Mobile TP'],pred1['Mobile TP']))


#Plotting Actual vs diaggregated data for the first 500 samples

xpoints = O_ts['Mobile TP'].values
xpoints = xpoints[0:500,]
xpoints1 = pred['Mobile TP']
xpoints1 = xpoints1[0:500,]


plt.plot(xpoints, 'o', label="Actual Mobile Power Consumption")
plt.plot(xpoints1, '*', label="Disaggregated Mobile Power Consumption")
plt.xlabel('Samples')
...
plt.show()
plt.draw()
fig1.savefig('Mobile.png', dpi=600,bbox_inches='tight')
